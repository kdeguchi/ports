--- src/VBox/HostDrivers/VBoxNetFlt/freebsd/VBoxNetFlt-freebsd.c.orig	2024-10-10 18:31:30 UTC
...
+
         for (m0 = m; m0 != NULL; m0 = m0->m_next)
             if (m0->m_len > 0)
                 cSegs++;
@@ -462,10 +485,40 @@ static void vboxNetFltFreeBSDinput(void *arg, int pend
...
+        }
     ...
     vboxNetFltRelease(pThis, true /* fBusy */);
     VBOXCURVNET_RESTORE();
@@ -478,12 +531,12 @@ static void vboxNetFltFreeBSDoutput(void *arg, int pen
 {
     PVBOXNETFLTINS pThis = (PVBOXNETFLTINS)arg;
     struct mbuf *m, *m0;
-    struct ifnet *ifp = pThis->u.s.ifp;
...
+    VBOXCURVNET_SET(if_getvnet(ifp)); /* XXX - IFAPI */
     ...
     for (;;)
     {
@@ -521,26 +574,33 @@ int vboxNetFltPortOsXmit(PVBOXNETFLTINS pThis, void *p
  */
...
@@ -571,9 +638,17 @@ int vboxNetFltPortOsXmit(PVBOXNETFLTINS pThis, void *p
...
@@ -586,15 +661,19 @@ int vboxNetFltOsInitInstance(PVBOXNETFLTINS pThis, voi
...
@@ -604,7 +683,7 @@ int vboxNetFltOsInitInstance(PVBOXNETFLTINS pThis, voi
...
@@ -638,16 +717,18 @@ int vboxNetFltOsInitInstance(PVBOXNETFLTINS pThis, voi
         vboxNetFltRelease(pThis, true /*fBusy*/);
     }
     VBOXCURVNET_RESTORE();
+    IPRT_FREEBSD_RESTORE_EFL_AC();
...
@@ -660,6 +741,7 @@ bool vboxNetFltOsMaybeRediscovered(PVBOXNETFLTINS pThi
         pThis->u.s.node = NULL;
     }
     VBOXCURVNET_RESTORE();
+    IPRT_FREEBSD_RESTORE_EFL_AC();
     ...
     if (ifp0 != NULL)
     {
@@ -672,6 +754,7 @@ void vboxNetFltOsDeleteInstance(PVBOXNETFLTINS pThis)
...
@@ -684,6 +767,7 @@ void vboxNetFltOsDeleteInstance(PVBOXNETFLTINS pThis)
...
@@ -697,7 +781,8 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
...
+    if_t ifp;
     ...
     int error;
     node_p node;
@@ -708,19 +793,19 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
...
@@ -730,9 +815,12 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
...
@@ -744,10 +832,13 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
         NG_MKMESSAGE(msg, NGM_GENERIC_COOKIE, NGM_CONNECT,
             sizeof(struct ngm_connect), M_NOWAIT);
         if (msg == NULL)
+        {
...
+            if_name(ifp));
         ...
         NG_SEND_MSG_PATH(error, node, msg, path, 0);
@@ -759,7 +850,7 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
         ifpromisc(ifp, 0);
...
@@ -767,7 +858,10 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
...
+        }
         ...
         NG_SEND_MSG_PATH(error, node, msg, path, 0);
@@ -778,12 +872,16 @@ void vboxNetFltPortOsSetActive(PVBOXNETFLTINS pThis, b
         NG_MKMESSAGE(msg, NGM_GENERIC_COOKIE, NGM_RMHOOK,
             sizeof(struct ngm_rmhook), M_NOWAIT);
         if (msg == NULL)
+        {
