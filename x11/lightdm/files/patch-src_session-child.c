--- src/session-child.c.orig	2024-06-20 04:53:29 UTC
+++ src/session-child.c
@@ -13,7 +13,6 @@
 #include <grp.h>
 #include <glib.h>
...
@@ -367,7 +344,6 @@ session_child_run (int argc, char **argv)
...
@@ -397,7 +373,7 @@ session_child_run (int argc, char **argv)
         else
         {
             /* Set POSIX variables */
...
@@ -755,7 +731,6 @@ session_child_run (int argc, char **argv)
             if (!pututxline (&ut))
...
@@ -796,7 +771,6 @@ session_child_run (int argc, char **argv)
             if (!pututxline (&ut))
                 g_printerr ("Failed to write utmpx: %s\n", strerror (errno));
             endutxent ();
-            updwtmpx ("/var/log/wtmp", &ut);
